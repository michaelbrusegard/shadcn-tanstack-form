...
const formSchema = z.object({
  ...
  numberField: z.number({ invalid_type_error: "Please enter a valid number." }),
  textAreaField: z.string().min(10, "Textarea must be at least 10 characters."),
  selectField: z
    .string()
    .refine(
      (val): val is "apple" | "orange" | "peach" | "melon" =>
        ["apple", "orange", "peach", "melon"].includes(val),
      "Please select a valid option.",
    ),
  radioGroupField: z
    .string()
    .refine(
      (val): val is "fiat" | "ferrari" | "astonMartin" | "mercedes" =>
        ["fiat", "ferrari", "astonMartin", "mercedes"].includes(val),
      "Please select a valid option.",
    ),
  ...
  return (
    ...
    >
      <form.AppForm>
        ...
        <form.AppField name="fieldProps">
          {(field) => (
            <field.TextField
              ...
              fieldSuffix=<span className="ml-2 text-xs">
                A ReactNode field suffix prop
              </span>
              labelSibling=<span className="ml-2 text-xs">
                A ReactNode label sibling prop
              </span>
            />
          )}
        </form.AppField>
        ...
        <form.SubmitButton className="mb-2 w-full min-w-32 sm:w-auto">
          Submit Button
        </form.SubmitButton>
      </form.AppForm>
    </form>
